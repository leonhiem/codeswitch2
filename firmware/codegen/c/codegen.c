...
{
    uint64_t code;
    unsigned int shsid,days,seq=0,runs=100,i,c;
    uint32_t chk_shsid;
    uint16_t chk_days;
    uint8_t chk_seq;
    char code_str[32];
    ...
    if(argc == 4) {
        ...
        fprintf(stderr,"Example: %s 372759817 5 1\n",argv[0]);
        fprintf(stderr,"(if seq not given, it prints 100 codes)\n");
        exit(EXIT_FAILURE);
    }

    days = atoi(argv[2]);
    shsid = atoi(argv[1]); // my codeswitch. Use code *1# to readout from keypad

    if(shsid>999999999 || days>999 || seq>99) {
        fprintf(stderr,"Number too large (shsid<1000000000 days<1000 seq<100)\n");
        exit(EXIT_FAILURE);
    }
    ...
    for(i=0;i<runs;i++) {
        ...
        printf("codegen id=%09u days=%03d seq=%02d ==> code = %09u%03u%02u -> %012lu%02u\n",
               shsid,days,seq,shsid,days,seqlut[seq],code,seqlut[seq]);
        sprintf(code_str,"%012lu%02u",code,seqlut[seq]);

        crypt_digits(code_str);

        printf("codegen crypted code = %s\n",code_str);
